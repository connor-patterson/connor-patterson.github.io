...
`PreferencesService` owns Day Shift, Night Shift, High Contrast, CRT texture intensity, motion, and Comfortable Reading. CRT texture intensity defaults to 100 and drives compositor-friendly scanline, fringe, and highlight overlays without geometrically filtering the application tree. Values are stored under a versioned local storage key. Storage failures are nonfatal.
