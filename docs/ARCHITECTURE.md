...
`PreferencesService` owns Day Shift, Night Shift, High Contrast, CRT texture intensity, motion, and Comfortable Reading. CRT texture intensity defaults to 100 and drives compositor-friendly scanline, fringe, and highlight overlays. A small pointer-following backdrop lens uses a static displacement map on desktop pointers only; the interactive application tree is never filtered or rerasterized. Values are stored under a versioned local storage key. Storage failures are nonfatal.
